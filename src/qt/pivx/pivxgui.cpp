...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Beacon client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Beacon"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Beacon"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
